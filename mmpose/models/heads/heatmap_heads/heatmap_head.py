...
from mmpose.evaluation.functional import pose_pck_accuracy #, atraf_classification_accuracy
...
class HeatmapHead(BaseHead):
    ...
    def predict(self,
        ...
        if test_cfg.get('flip_test', False):
            ...
            #_batch_heatmaps = self.forward(_feats) #BE: call wrong forward
            _batch_heatmaps = HeatmapHead.forward(self, _feats)  # BE:
            _batch_heatmaps_flip = flip_heatmaps(
                # self.forward(_feats_flip), #BE: call wrong forward
                HeatmapHead.forward(self,_feats_flip),  # BE:
                flip_mode=test_cfg.get('flip_mode', 'heatmap'),
                flip_indices=flip_indices,
                shift_heatmap=test_cfg.get('shift_heatmap', False))
            batch_heatmaps = (_batch_heatmaps + _batch_heatmaps_flip) * 0.5
        else:
            #batch_heatmaps = self.forward(feats) #BE: call wrong forward
            batch_heatmaps =  HeatmapHead.forward(self,feats)  # BE:
    ...
    def loss(self,
        ...
        """
        pred_fields = HeatmapHead.forward(self,feats) #BE
